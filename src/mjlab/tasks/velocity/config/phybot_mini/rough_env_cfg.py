...
@dataclass
class PhybotC1RoughEnvCfg(LocomotionVelocityEnvCfg):
  def __post_init__(self):
    super().__post_init__()
#碰撞传感器的设置：reduce="netforce"。mujoco会将所有碰撞汇总到sensordata的第一位，只要有碰撞第一位就是1,num则为最大碰撞检测数目
    ...
    feet_ground_cfg = ContactSensorCfg(
      ...
      primary=ContactMatch(
        mode="subtree",
        pattern=r"^(left_ankle_roll|right_ankle_roll)$",
        entity="robot",
      ),
    ...
    #相当于后续的场景只检测足底geom的话可以直接拿geom_name
    geom_names = []
    for i in range(1, 8):
    ...
    self.rewards.pose.params["std_moving"] = {
      # Lower body.
        ##惩罚是e^-error/std^2,越大限制越少，越小限制越严格
      r".*hip_pitch.*": 1.0,
      r".*hip_roll.*": 0.1,
      r".*hip_yaw.*": 0.2,
      r".*knee.*": 0.35,
...
@dataclass
class PhybotC1RoughEnvCfg_PLAY(PhybotC1RoughEnvCfg):
  def __post_init__(self):
    super().__post_init__()
