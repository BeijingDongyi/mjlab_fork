...
from mjlab.tasks.carrying.carrying_env_cfg import (
  CarryingEnvCfg,
)
...
@dataclass
class PhybotC1RoughCarryEnvCfg(CarryingEnvCfg):
  def __post_init__(self):
    super().__post_init__()
    foot_contact_sensors = [
      ContactSensorCfg(
        name=f"{side}_foot_ground_contact",
        body1=f"{side}_ankle_roll",
        body2="terrain",
        num=1,
        data=("found",),
        reduce="netforce",
    ...
    phybot_cfg = replace(phybot_ROBOT_CFG, sensors=tuple(foot_contact_sensors))
    self.scene.entities = {"robot": phybot_cfg,
...
@dataclass
class PhybotC1RoughCarryEnvCfg_PLAY(PhybotC1RoughCarryEnvCfg):
  def __post_init__(self):
    super().__post_init__()
